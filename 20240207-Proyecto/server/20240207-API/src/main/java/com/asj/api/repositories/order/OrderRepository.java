...
public interface OrderRepository extends JpaRepository<OrderModel, Integer> {
	...
	// H2
	@Query(value = "SELECT s.business_name, COUNT(o.supplier_id) AS orderCount " +
		"FROM orders o " +
		"JOIN suppliers s ON o.supplier_id = s.id " +
		"WHERE s.is_deleted = false " +
		"GROUP BY s.business_name " +
		"ORDER BY orderCount DESC, s.business_name ASC " +
		"LIMIT 5", nativeQuery = true)
	List<Object[]> findTopSuppliers();
	
	@Query(value = "SELECT EXTRACT(YEAR FROM issue_date) AS `year`, EXTRACT(MONTH FROM issue_date) AS `month`, SUM(total) " +
        "FROM orders " +
	...
	// MSSQL
	@Query(value = "SELECT TOP 5 s.business_name, COUNT(o.supplier_id) AS orderCount " +
        "FROM orders o " +
        "JOIN suppliers s ON o.supplier_id = s.id " +
        "WHERE s.is_deleted = false " +
        "GROUP BY s.business_name " +
        "ORDER BY orderCount DESC, s.business_name ASC", nativeQuery = true)
	List<Object[]> findTopSuppliers2();
	
	@Query(value = "SELECT TOP 12 YEAR(issue_date) AS `year`, MONTH(issue_date) AS `month`, SUM(total) " +
        "FROM orders " +

...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ErrorResponse> handleInvalidIdentifierException(InvalidIdentifierException ex) {
	...
	public ResponseEntity<ValidationErrorResponse> handleValidationErrorException(ValidationErrorException ex) {
		ValidationErrorResponse response = new ValidationErrorResponse(400, ex.getMessage(), ex.getErrors());
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
	}
	...
	public ResponseEntity<ErrorResponse> handleAssociatedEntitiesException(AssociatedEntitiesException ex) {
		ErrorResponse response = new ErrorResponse(409, ex.getMessage());
		return ResponseEntity.status(HttpStatus.CONFLICT).body(response);
	}

	@ExceptionHandler(UniqueViolationException.class)
	public ResponseEntity<ErrorResponse> handleUniqueViolationException(UniqueViolationException ex) {
		ErrorResponse response = new ErrorResponse(409, ex.getMessage());
		return ResponseEntity.status(HttpStatus.CONFLICT).body(response);
	}
